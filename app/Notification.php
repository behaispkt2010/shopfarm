...
class Notification extends Model
{
    ...
    protected $fillable = ['keyname','title','content','link', 'author_id', 'orderID_or_productID','roleview','is_read'];
    ...
    /*public static function GetNotifyCompany($strUserID){
        $join = Notification::leftjoin('users','notification.author_id','=','users.id')
            ...
            ->selectRaw('users.* ')
            ->selectRaw('company.* ')
            ->selectRaw('notification.created_at, notification.keyname, notification.orderID_or_productID, notification.title, notification.content, notification.roleview, notification.author_id, notification.link, notification.id')
            ->orderBy('notification.id','DESC')
            ->take(5)
            ->get();
        return $join;
    }
    public static function GetNotifyAdmin($strUserID){
